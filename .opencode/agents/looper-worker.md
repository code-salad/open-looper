...
# Verify required scripts exist (worker doesn't need claim-issue — dispatcher claims)
for script in setup-clone fetch-issue-context git-commit-loop cleanup-clone; do
...
echo "[worker] Task name: $TASK_NAME" >&2
```
...
if [ "$FETCH_EXIT" -eq 1 ]; then
    echo "[worker] Issue is blocked. Aborting." >&2
    exit 1
fi
...
echo "[worker] Working on issue #$ISSUE_NUMBER" >&2
```
...
while [ "$ITERATION" -le "$MAX_TDD_ITERATIONS" ]; do
    echo "[worker] TDD iteration $ITERATION/$MAX_TDD_ITERATIONS" >&2
...
MAX_TDD_ITERATIONS: $MAX_TDD_ITERATIONS") 2>&1) && DOER_EXIT=0 || DOER_EXIT=$?
    ...
    if [ "$DOER_EXIT" -ne 0 ]; then
        echo "[worker] Doer failed (exit $DOER_EXIT) on iteration $ITERATION. Aborting." >&2
        ABORTING="yes"
    ...
    if echo "$DOER_OUTPUT" | grep -q "ESCALATE:"; then
        echo "[worker] Doer escalated. Aborting loop." >&2
        ABORTING="yes"
        trap cleanup_on_abort EXIT
        exit 1
    fi
...
while [ "$ROUND" -le "$MAX_ROUNDS" ]; do
    echo "[worker] Review round $ROUND/$MAX_ROUNDS" >&2
...
MAX_ROUNDS: $MAX_ROUNDS") 2>&1) && REVIEW_EXIT=0 || REVIEW_EXIT=$?
    ...
    echo "[worker] Round $ROUND verdict: $VERDICT" >&2
    ...
    if [ "$VERDICT" = "FAIL" ]; then
        if [ "$ROUND" -eq "$MAX_ROUNDS" ]; then
            echo "[worker] Review failed on round $ROUND/$MAX_ROUNDS. Aborting." >&2
            ABORTING="yes"
            trap cleanup_on_abort EXIT
            exit 1
        fi
        ...
        echo "[worker] Review FAIL on round $ROUND. Re-running Doer for iteration $((ITERATION))..." >&2
...
MAX_TDD_ITERATIONS: $MAX_TDD_ITERATIONS") 2>&1) && DOER_EXIT=0 || DOER_EXIT=$?
        ...
        if [ "$DOER_EXIT" -ne 0 ]; then
            echo "[worker] Doer failed on re-run. Aborting." >&2
            ABORTING="yes"
            trap cleanup_on_abort EXIT
            exit 1
        fi
        ...
        if [ "$ITERATION" -gt "$MAX_TDD_ITERATIONS" ]; then
            echo "[worker] Max TDD iterations reached during reviewer retry. Aborting." >&2
            ABORTING="yes"
            trap cleanup_on_abort EXIT
            exit 1
        fi
    ...
    echo "[worker] ERROR: Could not determine verdict from reviewer. Aborting." >&2
    ABORTING="yes"
    trap cleanup_on_abort EXIT
    exit 1
done
...
```bash
echo "[worker] Review PASSED. Syncing with remote..." >&2
...
if [ "$SYNC_EXIT" -ne 0 ]; then
    echo "[worker] Sync failed (exit $SYNC_EXIT). Resolve conflicts manually." >&2
    exit 1
fi

# Create PR
echo "[worker] Creating PR..." >&2
...
echo "[worker] PR creation complete (exit $EXIT_CODE)" >&2
exit $EXIT_CODE
```
...
cleanup_on_abort() {
    ...
    if [ "$ABORTING" = "yes" ] && [ -n "$CLONE_DIR" ] && [ -d "$CLONE_DIR" ]; then
        echo "[worker] Cleaning up clone on abort..." >&2
